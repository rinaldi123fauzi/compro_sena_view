...
// Computed
const projects = computed(() => searchResults.value?.data?.projects || []);
const news = computed(() => searchResults.value?.data?.news || []);
const totalResults = computed(
    () => (projects.value.length || 0) + (news.value.length || 0)
);
...
// On mounted - check for initial query
onMounted(() => {
